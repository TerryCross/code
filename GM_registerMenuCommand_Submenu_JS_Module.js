...
//
//    submenuModule.register("my script's menu cmd name", hotkey );
//
...
var submenuModule=(function() { //a module, js pattern module, ownSubmenu() is a closure returning an interface object.  Side effect alters GM_registerMenuCommand.
	...
	if (window.chrome) {
	    old_GM_registerMenuCommand=function(){};
	    setUpChromeButton();
	}
	...
	$(window).on("keydown",function(e) { if (e.altKey&&e.keyCode==altHotkey) {  openSubmenu(); return false;}}); // alt-m or hotkey
	$(docready);
	state="init";
	if (iframe) return;
	...
	li.click(function(e){ closeSubmenu(true);
			      body.trigger("click");
			      setTimeout(func,100);} );
	...
	ownSubmenu.on("focus.osm",function(e){ ownSubmenuList.focus();});
	...
	if (now===true) { ownSubmenu.hide(); document.dispatchEvent(new Event("coord_resize"));}
	else ownSubmenu.hide(400,function() {
	    document.dispatchEvent(new Event("coord_resize")); });
	ownSubmenu.off(".osm");
	...
	var code=GM_getValue ? GM_getValue("osm_jqueryCode","") : "";
	if (!code) {
	    code=httpGet("https://code.jquery.com/jquery-latest.js");
	    if (GM_setValue) GM_setValue("osm_jqueryCode",code);
	}
	eval(code);
	...
	var right_pos=GM_getValue ? GM_getValue("GMmenuLeftRight", true) : localStorage.GMmenuLeftRight;
	...
	if (div.length==0) {
		...
		if (e.button==0) {
		    ...
		    // GM_setValue("GMmenuLeftRight", ( this.parentNode.style.right ? true : false )
		} 
	    }, false);
	...
	function unsafeWindowObj() { //a singleton object.
		...
		var newev=new window.CustomEvent("Storage-changed",{detail:{from_id:coord_id,newval:this.underlying_obj}});
		document.dispatchEvent(newev);// target is document
	    };  
	...
	if (! document.getElementById("squadafont")) {
	    var css=GM_getValue && GM_getValue("squada","");
	    if (!css) {
		...
		httpGet("https://fonts.googleapis.com/css?family=Squada+One",function(e){
			...
			if (GM_setValue) GM_setValue("squada",css);
			injectCss(css);
		    };
